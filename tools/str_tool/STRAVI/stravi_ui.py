...
from str_converter import convert_str_to_avi, convert_avi_to_str

class STRAVITool:
    def __init__(self, root):
        self.root = root
        self.root.title("STRAVI - STR to AVI Converter")
    ...
    def convert(self):
        ...
        output_path = filedialog.asksaveasfilename(defaultextension=".avi" if self.mode.get() == "STR to AVI" else ".str")
        if not output_path:
        ...
        if self.mode.get() == "STR to AVI":
            convert_str_to_avi(self.file_path, output_path, logger=self.log)
        else:
            convert_avi_to_str(self.file_path, output_path, logger=self.log)
